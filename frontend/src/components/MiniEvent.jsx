function MiniEvent({event, onUpdate, onDelete}) {
    ...
    return <div className="flex gap-0 pb-2
    ">
        <div className={`w-1 ${categoryColor} rounded-full`}></div>
            <div className="flex-1">
                <div className="flex flex-wrap justify-between items-start">
                    <div>
                        <h3 className=" text-xs md:text-sm font-semibold">{event.title}</h3>
                        <p className="text-slate-600 text-xs md:text-sm">{event.time}</p>
                        {/* <p className="text-slate-500 mt-1">
                            {event.content}
                        </p> */}
                    </div>
                <div className="flex flex-col gap-1">
            <button className="text-blue-500 text-xs text-left md:text-right" onClick={() => onUpdate(event.id)}>Edit</button>
            <button className="text-red-500 text-xs text-left md:text-right" onClick={() => onDelete(event.id)}>Delete</button>
            </div>
        </div>
    </div>
</div>
